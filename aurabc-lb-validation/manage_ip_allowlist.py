...
POC tool for adding/removing NAT Gateway IPs during load balancer validation testing.

Usage:
    uv run python manage_ip_allowlist.py list
    uv run python manage_ip_allowlist.py add --ip 20.42.0.10 --description "NAT Gateway IP"
...
def add_ip_to_allowlist(ip_address, description):
    ...
    # Name must be <= 30 chars
    name = f"LB-POC-{ip_address}"[:30]
    payload = {
        "name": name,
