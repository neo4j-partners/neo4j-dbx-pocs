...
Connects Databricks serverless compute to Neo4j Aura Business Critical over Azure Private Link using a load balancer and reverse proxy architecture. Validated end-to-end on 2026-03-19.
...
Databricks Serverless (eastus)
    |
    |  NCC private endpoint (cross-region)
    v
...
The NCC private endpoint can be in a different region than the Private Link Service. In this deployment, the Databricks workspace is in eastus and the Azure infrastructure is in westus3.

## How it works

- **Network Connectivity Configuration (NCC):** A Databricks account-level resource that controls how serverless compute makes outbound network connections. The NCC must be created in the same region as the Databricks workspace, not the Azure infrastructure region. Set `NCC_REGION` in `.env` when these differ.
...
| `NCC_REGION` | Region for the NCC (must match workspace region, e.g. `eastus`). Defaults to `AZURE_LOCATION` if unset. | Databricks Workspace settings or URL |
...
> **Important:** The NCC must be in the same Azure region as the Databricks workspace, which may differ from the Azure infrastructure region (`AZURE_LOCATION`). Set `NCC_REGION` in `.env` if your workspace is in a different region than the infrastructure. The private endpoint rule can point cross-region to the PLS without issues.
...
Deploys a test VM in the Databricks region (eastus) with a private endpoint to the PLS in the infra region (westus3), simulating the exact cross-region path that Databricks serverless takes. Runs pytest to validate the full chain end-to-end.
...
- **NCC region must match workspace region.** The NCC can point cross-region to a PLS, but the NCC itself must be in the same region as the workspace.
