"""
Deploy and run the Aura BC private link validation test VM.

Workflow (3 commands):
    uv run python deploy_test_vm.py deploy    — Deploy test VM, SCP test files
...
Reads all configuration from the parent directory's .env and
deployment-outputs.json — no manual parameter editing required.
"""
...
RESOURCE_GROUP = os.getenv("AZURE_TEST_RESOURCE_GROUP", "aurabc-test-rg")
LOCATION = os.getenv("AZURE_TEST_LOCATION", "eastus")
PREFIX = os.getenv("AZURE_TEST_PREFIX", "aurabc-test")
DEPLOYMENT_NAME = "aurabc-test-vm"
BICEP_FILE = os.path.join(BASE_DIR, "infra", "main.bicep")
OUTPUTS_FILE = os.path.join(BASE_DIR, "test-vm-outputs.json")
ADMIN_USER = "azureuser"
REMOTE_DIR = "/opt/aurabc-test"

# Files to SCP to the VM
TEST_FILES = ["pyproject.toml", "conftest.py", "test_lb_connectivity.py"]
...
def load_parent_outputs():
    """Load deployment-outputs.json from the parent LB infrastructure."""
    path = os.path.join(PARENT_DIR, "deployment-outputs.json")
    if not os.path.exists(path):
        print(f"ERROR: {path} not found. Deploy the LB infrastructure first.")
        sys.exit(1)
    with open(path) as f:
        return json.load(f)
...
def resolve_params():
    """Build Bicep parameters from parent .env and deployment-outputs.json."""
    parent_outputs = load_parent_outputs()
    pls_id = parent_outputs.get("privateLinkServiceId", "")
    if not pls_id:
        print("ERROR: privateLinkServiceId not found in deployment-outputs.json")
        sys.exit(1)
...
def generate_vm_env(vm_outputs):
    ...
    env_lines = [
        ...
        f"NAT_GW_IP={parent_outputs.get('natGatewayPublicIp', '')}",
...
def query_pe_ip(pe_name):
    ...
    nic_id = pe.strip()
    # Query the NIC for its private IP
    ip = run_az(
...
def cmd_deploy():
    ...
    print(f"\n  PLS: {params['plsResourceId']['value']}")
    ...
    # Deploy Bicep
    print("\n[2/5] Deploying Bicep template (3-5 minutes)...")
    result = run_az(
    ...
    print("\n[3/6] Fetching deployment outputs...")
    outputs = run_az(
    ...
    # Query the PE NIC to get the private IP (not available in Bicep outputs
    # for PLS endpoints — customDnsConfigs is empty)
    print("\n[4/6] Querying private endpoint IP...")
    pe_ip = query_pe_ip(pe_name)
    print(f"  PE Private IP: {pe_ip}")

    # Add PE IP to flat outputs for saving
    flat["privateEndpointIp"] = pe_ip
    with open(OUTPUTS_FILE, "w") as f:
        json.dump(flat, f, indent=2)

    # Generate .env for the VM
    print("\n[5/6] Generating VM environment...")
    generate_vm_env(flat)

    # Wait for VM, configure /etc/hosts, and SCP files
    print("\n[6/6] Setting up test environment on VM...")
    wait_for_ssh(vm_ip)
...
def cmd_test():
    """Run pytest on the VM via SSH. Full output goes to test-output.log."""
    ...
    log_path = os.path.join(BASE_DIR, "test-output.log")
    ...
    ssh_opts = "-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR"
    test_cmd = f"cd {REMOTE_DIR} && /usr/local/bin/uv run pytest -v -s --tb=short 2>&1"
    ...
    # Print summary to console: header lines + PASSED/FAILED/ERROR lines + footer
    for line in result.stdout.splitlines():
        ...
        if (
            ...
            or stripped.startswith("collecting")
...
def main():
    parser = argparse.ArgumentParser(
        description="Deploy and test the Aura BC private link validation VM",
